...
class TJALFirstInstanceHttpCrawler(BaseHttpCrawler):

    def __init__(self, timeout: int = 30):
        super().__init__("TJAL")
        self.timeout = timeout
        self.base_url = "https://www2.tjal.jus.br"
        self.url_busca = f"{self.base_url}/cpopg/search.do"
        self.headers = {
            "User-Agent": (
    ...
    async def run(self, cnj: str) -> Optional[str]:
        ...
        if not match:
            logger.warning(f"CNJ inválido: {cnj}")
            return None
        ...
        async with httpx.AsyncClient(timeout=self.timeout, follow_redirects=True) as client:
            try:
                logger.info(f"Searching for {cnj} process in TJAL 1st instance")
                ...
                if input_tag:
                    ...
                    url_final = f"{self.base_url}/cpopg/show.do?processo.codigo={processo_codigo}"
